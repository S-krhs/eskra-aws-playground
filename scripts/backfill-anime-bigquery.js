// Invokes the anime-metric BigQuery export Lambda one calendar month at a time over a given period.
// The period is split so a single invoke fits inside Lambda's 15-minute execution time, and a failure
// partway reports how far it got so the rest can be re-run.
...
/** Converts YYYY-MM-DD to a Date at UTC 00:00; a nonexistent date is refused. */
function parseDate(value, label) {
	if (!/^\d{4}-\d{2}-\d{2}$/.test(value ?? "")) {
		throw new Error(`${label}が YYYY-MM-DD 形式ではありません: ${value}`);
	}

	// Date rolls a nonexistent day like 2026-02-30 forward, so round-trip it and compare
...
/** Splits a period on calendar-month boundaries; inclusive at both ends. */
...
/** Checks the export Lambda exists, stopping here when the name has drifted. */
...
/** Invokes one interval and returns the Lambda's response. */
	...
	try {
		const metadata = execFileSync(
			...
			[
				...
				"--cli-binary-format",
				"raw-in-base64-out",
				// The default 60-second read timeout can't wait out a 15-minute run
				"--cli-read-timeout",
				"0",
				outputPath,
			],
...
/** Writes the results table into GitHub Actions' Job Summary. */
...
function main() {
	...
	for (const [index, chunk] of chunks.entries()) {
		...
		try {
			...
			// Finished intervals stay on the BigQuery side, so a re-run can start from the failed one
			writeSummary([
				"## BigQuery 連携（失敗）",
				"",
