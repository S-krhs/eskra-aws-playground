...
| 層 | 置くもの | 置かないもの |
| --- | --- | --- |
| `src/handlers/` | Lambda エントリポイント（`batch.ts`: batch 共通、`function-url.ts`: HTTP リクエストを受ける Function URL 公開エンドポイント）、開始/終了ログ | ジョブ選択条件、業務ロジック、外部連携詳細 |
...
function-url -> jobs -> routing(interaction-router) -> features
