...
- ダブルクリックで操作を起こすとき `onDoubleClick` に頼らない。ブラウザの `dblclick` は click 回数がちょうど 2 のときしか出ず、連打すると 2 回目以降が出ない（Chrome は 3 まで数えてから 1 に戻すため、6 連打で 2 回しか出ない）。クリック時刻を自前で持ち、一定間隔以内の 2 回目で起こして数え直す。
- 最小化した窓の並び位置を、最小化中の窓の配列の添字で表さない。1 枚を復元すると残りの添字がずれ、下辺のバーが左へ詰まる。添字を位置、値を窓とする配列で持ち、空いた位置は `null` のまま残して次の最小化で使い回す。
