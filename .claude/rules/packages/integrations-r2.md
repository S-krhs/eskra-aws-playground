...
- secret の JSON 文字列を受ける入口は `parseR2CredentialsJson` に置く。`JSON.parse` の `SyntaxError` は入力の先頭を message に含めるため、app 側で `JSON.parse` させると secret がログへ流れる(`integration-bigquery` の `parseServiceAccountKey` と同じ扱い)。
