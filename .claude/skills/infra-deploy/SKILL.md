---
name: infra-deploy
description: Rules for SST config, GitHub Actions workflows, and CI helper scripts — touch this when editing infra/**, .github/workflows/**, or scripts/**.
---
...
- Secrets have exactly one source: the stage's SST secret. CD writes it as `SST_SECRET_*` env during deploy; runtime code reads it via `sst shell` (scripts) or `Resource` (Lambda). Never pass a secret as a plain env var.
