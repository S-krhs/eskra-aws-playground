...
- A value the app must never construct lives in that repository's `internal/`, closed off in `package.json` exports. **Give such an entry an explicit base rather than a wildcard one** (`./media/media-storage/internal/*.js`, not `./media/*/internal/*.js`): two patterns whose text before the first `*` is the same length can resolve either way, and the block silently stops working.
