...
8. `npm run deploy`
9. `sst shell --stage develop ... npm run discord:sync:run`（宣言済みスラッシュコマンドを Discord guild へ同期。SST secret を参照するため sst shell 経由。deploy 後に実行する）
...
- `DISCORD_APPLICATION_ID`（スラッシュコマンド同期先の Discord application ID。deploy step で `SST_SECRET_DiscordApplicationId` として SST secret に渡す）
- `DISCORD_GUILD_ID`（スラッシュコマンド同期先の Discord guild ID。deploy step で `SST_SECRET_DiscordGuildId` として SST secret に渡す）

スラッシュコマンド同期に必要な 3 値（bot token・application ID・guild ID）は deploy step で `SST_SECRET_*` として SST secret に書き込み、同期 step は `sst shell` 経由で SST secret から読みます。同期 step 自体には secret を env で渡しません。`DISCORD_BOT_TOKEN` は `SST_SECRET_DiscordBotToken` と同値の既存 Secret を共用します。
...
Yaccho Bot のスラッシュコマンドは [commands.ts](../apps/batch-playground/src/handlers/function-url/routes/yaccho-bot-interaction/shared/commands.ts) で定義し、CD の deploy 後に Discord guild へ `PUT .../commands`（bulk overwrite）します。定義に無いコマンドは Discord 側から削除されるため、コードと登録コマンドが常に一致します。冪等なので毎 deploy 実行して問題ありません。

コマンドの追加・変更は `commands.ts` の編集だけで完結し、`main` への merge で自動反映されます。

同期スクリプトは bot token・application ID・guild ID を SST secret（`Resource`）から読むため、`sst shell` 経由で起動します。secret を env で直接渡さないので、他の app と同じく stage ごとの SST secret が唯一の供給元になります。

- CD（develop stage）: deploy step が 3 値を `SST_SECRET_*` で SST secret に書き込み、同期 step が `sst shell --stage develop ... -- npm run discord:sync:run` で参照します。
...
- [Discord Developer Portal](https://discord.com/developers/applications) で application を作成し、Bot を追加する。
- **Application ID**・**Public Key**（General Information）と Bot **Token**（Bot タブ）を控える。
- OAuth2 URL Generator で scope に `bot` と `applications.commands` を付けた招待 URL を生成し、対象 guild へ bot を招待する（`applications.commands` が無いとスラッシュコマンドが表示されない）。
- CD 用に GitHub Secrets へ `DISCORD_BOT_TOKEN`・`DISCORD_APPLICATION_ID`・`DISCORD_GUILD_ID` を登録する（deploy step が `SST_SECRET_*` として develop stage の SST secret に書き込む）。
- ローカルで同期する stage には SST secret を直接設定する: `npx sst secret set DiscordBotToken <token>` / `DiscordApplicationId <app-id>` / `DiscordGuildId <guild-id>` / `DiscordInteractionPublicKey <public-key>`。
- 初回 deploy 後、出力された Function URL に `/discord/interactions/yaccho-bot` を付けた URL を Portal の **Interactions Endpoint URL** に設定する。設定時に Discord が検証リクエストを送るため、エンドポイントが deploy 済みで動作している状態でのみ保存できる（この 1 回のみ手動、以降 URL は変わらない）。
