...
`ListObjectsV2` は custom metadata を返しません。そのため同期は次の手順を取り、`findAllKeys` はそのための射影です。

1. R2 の key を全件列挙する
2. `findAllKeys` の結果と突き合わせ、既知の key はそのまま照合する
3. 未知の key にだけ `HeadObject` を打ち、metadata の UUID で移動か新規かを判定する

## media-object

- `findAllKeys`: 登録済みの id と key だけを全件返す。同期の突き合わせ用で、本文の列は読まない。
- `findById`: id で 1 件返す。ゴミ箱に入れたものも返す。
- `findPage`: 新着順の keyset pagination。ゴミ箱に入れたものは除外する。
- `insertMany` / `relocateMany` / `touchMany` / `deleteByIds`: 同期が差分を反映するための書き込み。
...
- `findRunning`: 終了していない実行。あれば起動を弾く。
